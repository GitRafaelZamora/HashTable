...
// Inserts a value using linear probing for collision resolution.
...
// returns a key for the linearInsert function
int hashClass::linearHash(int value) {
	return floor(this->table_size * (fmod((this->alpha * value), 1)));
}

// Runs through the linear probing sequence to return a string.
